...
# - Ejecuta tools/adsb/fetch_adsbfi.py cada 1 s (LEBL, 80 NM) para poblar aircraft.json.
...
CO_ATC_DIR="$ROOT_DIR/co-atc-main"
CONFIG="$CO_ATC_DIR/configs/config.toml"
CO_ATC_BIN="$CO_ATC_DIR/bin/co-atc"
LOG_DIR="$ROOT_DIR/logs"
FETCHER="$ROOT_DIR/tools/adsb/fetch_adsbfi.py"
...
(cd "$ROOT_DIR" && python3 "$FETCHER" --lat 41.2971 --lon 2.0785 --dist 80 --interval 1 >>"$LOG_DIR/adsbfi_fetch.log" 2>&1) &
FETCH_PID=$!
