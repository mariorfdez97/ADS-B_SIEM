...
type SimulationService interface {
	...
	GetFlightPlan(hex string) []Position        // Returns flight plan for simulated aircraft
}
	...
	for _, raw := range rawData.Aircraft {
		...
		if raw.Lat != 0 && raw.Lon != 0 && raw.AltBaro != 0 {
			...
			if heading != 0 && speed != 0 {
				// Check if we have a simulated flight plan
				var futurePredictions []Position
				if isSimulated && s.simulationService != nil {
					flightPlan := s.simulationService.GetFlightPlan(raw.Hex)
					if len(flightPlan) > 0 {
						futurePredictions = flightPlan
					}
				}

				// If no flight plan, calculate future positions
				if len(futurePredictions) == 0 {
