...
const (
	MaxSimulatedAircraft = 100 // Increased limit to accommodate anomalies and multiple flights
)
...
type SimulatedAircraft struct {
	...
	FlightPlan         []adsb.Position `json:"flight_plan"`
...
func (s *Service) CreateAircraft(lat, lon, altitude, heading, speed, verticalRate float64, flightPlan []adsb.Position) (*SimulatedAircraft, error) {
	s.mutex.Lock()
...
func (s *Service) updateAircraftPosition(aircraft *SimulatedAircraft, deltaTime float64) {
	...
	// Debug log to track position updates and detect jumps
	s.logger.Debug(fmt.Sprintf("UpdatePos hex=%s lat=%.6f lon=%.6f alt=%.0f", aircraft.Hex, aircraft.CurrentLat, aircraft.CurrentLon, aircraft.CurrentAltitude))
}
