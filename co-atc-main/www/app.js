        ...
        async fetchAircraftData() {
            try {
                ...
                for (const [hex, aircraft] of Object.entries(newAircraftMap)) {
                    ...
                    // CRITICAL FIX: Automatically remove simulated aircraft that are signal_lost
                    // This prevents "ghost" aircraft from persisting after simulation ends
                    if (aircraft.status === 'signal_lost' && (aircraft.type === 'sim' || (aircraft.flight && aircraft.flight.startsWith('SIM')))) {
                        delete this.aircraft[hex];
                        if (this.mapManager) {
        ...
        handleStatusUpdateMessage(data) {
            if (data && data.hex) {
                ...
                if (data.new_status === "signal_lost") {
                    ...
                    if (aircraft && (aircraft.type === 'sim' || (aircraft.flight && aircraft.flight.startsWith('SIM')))) {
                        console.log(`Simulated aircraft ${data.hex} signal lost - removing immediately`);
                        this.handleAircraftRemoved({ hex: data.hex });
                        return;
                    }
                }
